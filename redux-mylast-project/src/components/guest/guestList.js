...
function GuestList() {
    ...
    useEffect(() => {
        dispatch(fetchGuestList(curpage));
    }, [curpage]);

    const guestList=useSelector(state=>state.guests.guest_list)
                            ...
                            <div className="hotel-grid">
                                {guestList.list.map((guest) => (
                                    <div className="hotel-card" key={guest.no}>
                                        <img src={"https://korean.visitseoul.net"+guest.poster} alt={guest.title} className="hotel-image"
                                             onError={(e) => {
